...
public class TypeVisitor extends Visitor {
    ...
    public void visit(Tree.BaseType that) {
        ...
        TypeDeclaration type = that.getScope().getType(name, that.getUnit());
        if (type==null) {
            that.addError("type declaration does not exist: " + name, 102);
            unit.getUnresolvedReferences().add(that.getIdentifier());
        }
        else {
//            if (!type.isDefinedInScope(that.getScope())) {
//                that.addError("inherited types must be fully qualified in type expressions");
//            }
//            else {
//                ProducedType outerType = type.getType().getQualifyingType();
            ProducedType outerType = null;
            if (!(type instanceof TypeParameter) &&
                    type.isClassOrInterfaceMember()) {
                Scope scope = that.getScope();
                while (!(scope instanceof Package)) {
                    if (scope instanceof ClassOrInterface) {
                        ClassOrInterface ci = (ClassOrInterface) scope;
                        if (ci.inherits((ClassOrInterface)type.getContainer())) {
                            outerType = ci.getType();
                            break;
                        }
                    }
